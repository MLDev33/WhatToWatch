...
/**
 * Route pour sauvegarder les paramètres d'une liste à sa création pour media type "Movies"
 * Nota l'ajout des media est traité par une fonction externe dasn un deuxième temps
 * avec la fonction addMediaMovie et addMediaTV
 * 
 * @param { token }
 * 
 */
router.post('/add/movie/:token', async (req, res) => {
        ...
        User.findOne({ token: req.params.token.toString() })
            .then(async data => {
                if (data) {
                    ...
                    //const moviesListsMovie = await addMediaMovies(req.body, 1)
                    //const moviesListsTV = await addMediaTv(req.body, 1)
                    const mediaLists = await addMediaMovies(req.body, 1)
                    ...
                    const newMovieLists = new MovieLists({
                        ...
                        options:
                        {
                            ...
                            // average, releaseDateGte à rajouter au schema initial de movielists ?
                    ...
                    console.log("moviesLists", mediaLists)
                    newMovieLists.save().then(newDoc => {
                        ...
                        console.log("movies:", mediaLists)
                        res.status(200).json({ result: true, result: newDoc })
                    })
                }
                else {
...
/**
 * Route pour sauvegarder les paramètres d'une liste à sa création pour media type "TV"
 * Nota l'ajout des media est traité par une fonction externe dasn un deuxième temps
 * avec la fonction addMediaMovie et addMediaTV
 * 
 * @param { token }
 * 
 */
router.post('/add/tv/:token', async (req, res) => {
        ...
        User.findOne({ token: req.params.token.toString() })
            .then(async data => {
                if (data) {
                    ...
                    //const moviesListsMovie = await addMediaMovies(req.body, 1)
                    //const moviesListsTV = await addMediaTv(req.body, 1)
                    const mediaLists = await addMediaTv(req.body, 1)
...
async function addMediaMovies(filters, pagefilter) {
    ...
    let page = pagefilter;
    let type = "movie"; // variable override => pb d'utilisation de Promise.all([ , ]) refaire deux...
    genres = isEmpty(genres) ? "" : genres.map(genre => { return genre.id }).slice(",").join("|");
    providers = isEmpty(providers) ? "" : providers.length === 1 ? providers[0].toString() : providers.slice(",").join("|");
 ...
 * @returns un tableau d'objets contenant les media proposés à l'utilisateurs
 */
async function addMediaTv(filters, pagefilter) {
    ...
    let page = pagefilter;
    let type = "tv"; // variable override => pb d'utilisation de Promise.all([ , ]) refaire deux...
    genres = isEmpty(genres) ? "" : genres.map(genre => { return genre.id }).slice(",").join("|");
    providers = isEmpty(providers) ? "" : providers.length === 1 ? providers[0].toString() : providers.slice(",").join("|");
    average = isEmpty(average) ? "" : Number.parseInt(average);
    ...
    const listMediaTV = data.results.map((item) => {
        //const movieId = item.id;
        //const { listProviders, watchLink } = await getMoviesProviders(movieId, BASE_URL, API_KEY)
        const releaseDateGte = moment(item.release_date || item.first_air_date).format('YYYY-MM-DD');
...
router.delete("/delete/:id/:token", async (req, res) => {
    ...
    try {
        ...
        MovieLists.deleteOne({ _id: req.params.id }).then(() => {
            MovieLists.find().then(data => {
                console.log(data)
            })
        })
...
// Route à finir reste à récupérer l'id de la lste dans la population "created_list"
// puis remettre la route en DELETE à la place de get 
// ou transformer la route en fonction delete à ajouter à la route delete movieList
router.get('/user/delete/list/:token/:id', async (req, res) => {

    try {
        const user = await User.findOne({ token: req.params.token.toString() }).populate("created_list");
        console.log("user populate created_list:", user)
        if (!user) {
            return res.status(404).json({ success: false, message: "Utilisateur non trouvé" });
        }
